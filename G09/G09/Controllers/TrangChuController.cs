...
namespace G09.Controllers
{
    ...
    public class TrangChuController : Controller
    {
        ...
        public IActionResult ListBaiViet()
        {
            ...
            {
                _logger.LogWarning("Email người dùng không tồn tại trong session.");
                return Content("Email người dùng không tồn tại trong session.");
            }
            us = _context.NguoiDungs
               .FirstOrDefault(t => t.Email == currentUserEmail);
            if (us == null)
            {
                _logger.LogWarning("Người dùng không tồn tại trong cơ sở dữ liệu.");
